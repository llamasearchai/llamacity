...
class EncoderConfig:
    ...
    positional_encoding_type: str = (
        "learned"  # Options: sinusoidal, learned, rotary, relative, alibi
    )
    ...
    def to_dict(self) -> Dict:
        """Convert config to dictionary for serialization."""
        config_dict = {
            k: v for k, v in self.__dict__.items() if k != "attention_config"
        }
...
class DecoderConfig(EncoderConfig):
    ...
    def from_dict(cls, config_dict: Dict) -> "DecoderConfig":
        ...
        config = cls(**config_dict)
        config.attention_config = AttentionConfig.from_dict(attention_config)
        config.cross_attention_config = AttentionConfig.from_dict(
...
class FusionConfig:
    ...
    fusion_type: str = (
        "cross_attention"  # Options: cross_attention, concat, weighted, gated
    )
    hidden_size: int = 768
    ...
    def to_dict(self) -> Dict:
        """Convert config to dictionary for serialization."""
        config_dict = {
            k: v for k, v in self.__dict__.items() if k != "cross_attention_config"
        }
        config_dict["cross_attention_config"] = self.cross_attention_config.to_dict()
        return config_dict
...
class MultiModalConfig:
    ...
    pooling_type: str = (
        "attentional"  # Options: attentional, cls, mean, max, hierarchical
    )
    pooling_heads: int = 8
    ...
    def from_dict(cls, config_dict: Dict) -> "MultiModalConfig":
        ...
        image_encoder = (
            EncoderConfig.from_dict(image_encoder_dict) if image_encoder_dict else None
        )

        audio_encoder_dict = config_copy.pop("audio_encoder", None)
        audio_encoder = (
            EncoderConfig.from_dict(audio_encoder_dict) if audio_encoder_dict else None
        )

        video_encoder_dict = config_copy.pop("video_encoder", None)
        video_encoder = (
            EncoderConfig.from_dict(video_encoder_dict) if video_encoder_dict else None
        )
...
class ModelConfig:
    ...
    model_type: str = (
        "encoder_only"  # Options: encoder_only, encoder_decoder, decoder_only
    )
    ...
    def __post_init__(self):
        ...
        if (
            self.model_type in ["encoder_only", "encoder_decoder"]
            and not self.encoder_config
        ):
    ...
    def from_dict(cls, config_dict: Dict) -> "ModelConfig":
        ...
        encoder_config = (
            EncoderConfig.from_dict(encoder_config_dict)
            if encoder_config_dict
            else None
        )

        decoder_config_dict = config_copy.pop("decoder_config", None)
        decoder_config = (
            DecoderConfig.from_dict(decoder_config_dict)
        ...
        multi_modal_config = (
            MultiModalConfig.from_dict(multi_modal_config_dict)
            if multi_modal_config_dict
            else None
        )
    ...
    def create_default_multi_modal_config(
        self,
        ...
        multi_modal_config = MultiModalConfig(text_encoder=deepcopy(base_encoder))
