...
class LinearAttention(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        if self.dim > 0:
            nn.init.normal_(
                self.q_map.weight, mean=0, std=self.q_map.weight.shape[0] ** -0.5
            )
            nn.init.normal_(
                self.k_map.weight, mean=0, std=self.k_map.weight.shape[0] ** -0.5
            )
    ...
    def forward(
        ...
        if self.causal:
            ...
            cumulative_k = torch.zeros(
                batch_size,
                self.num_heads,
                1,
                self.dim if self.dim > 0 else self.head_dim,
                device=q.device,
            )
            cumulative_kv = torch.zeros(
                batch_size, self.num_heads, 1, self.head_dim, device=q.device
            )
            ...
            for i in range(seq_len_k):
                ...
                cumulative_kv = cumulative_kv + torch.matmul(
                    k_i.transpose(1, 2), v_i.transpose(1, 2)
                )
                ...
                if i < seq_len:
                    ...
                    q_i = (
                        q[:, i].unsqueeze(1).transpose(1, 2)
                    )  # [batch_size, num_heads, 1, dim]

                    # Compute attention using cumulative sums
                    normalizer = (
                        torch.matmul(q_i, cumulative_k.transpose(2, 3)) + self.eps
                    )
                    output_i = (
                        torch.matmul(cumulative_kv, q_i.transpose(2, 3)) / normalizer
                    )
        ...
        if self.causal:
            ...
            for i in range(seq_len_k):
                ...
                if i < seq_len:
                    ...
                    # Compute attention using cumulative sums
                    normalizer = (
                        torch.matmul(q_i, cumulative_k.transpose(1, 2).unsqueeze(1))
                        + self.eps
                    )
                    output_i = (
                        torch.matmul(q_i, cumulative_kv.unsqueeze(1)) / normalizer
                    )
