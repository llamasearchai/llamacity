...
class SparseMultiHeadAttention(nn.Module):
        ...
        for i in range(num_windows):
            ...
            if len(window_indices) < self.window_size:
                padding = torch.arange(
                    end_idx - 1,
                    end_idx - 1 - (self.window_size - len(window_indices)),
                    -1,
                )
    ...
    def _global_local_attention(
        ...
        q_global = q_global.transpose(
            1, 2
        )  # [batch_size, num_heads, global_tokens, head_dim]
        ...
        if self.causal:
            causal_mask = torch.triu(
                torch.ones(self.global_tokens, seq_len, device=device) * float("-inf"),
                diagonal=1,
            )
            global_attn_weights = global_attn_weights + causal_mask.unsqueeze(
                0
            ).unsqueeze(0)

        # Apply attention mask for global tokens if provided
        if attention_mask is not None:
            global_mask = attention_mask.unsqueeze(1).unsqueeze(
                2
            )  # [batch_size, 1, 1, seq_len]
            global_attn_weights = global_attn_weights.masked_fill(
                global_mask == 0, float("-inf")
            )
        ...
        for i in range(num_windows):
            ...
            window_q = q_local[
                :, window_q_indices
            ]  # [batch_size, window_q_len, num_heads, head_dim]
            window_q = window_q.transpose(
                1, 2
            )  # [batch_size, num_heads, window_q_len, head_dim]
            ...
            window_k = window_k.transpose(
                1, 2
            )  # [batch_size, num_heads, window_size, head_dim]
            window_v = window_v.transpose(
                1, 2
            )  # [batch_size, num_heads, window_size, head_dim]

            # Compute window attention scores
            window_attn_weights = (
                torch.matmul(window_q, window_k.transpose(2, 3)) * self.scale
            )
            ...
            if self.causal:
                ...
                rel_pos = window_idx.unsqueeze(0) - (
                    self.global_tokens + window_q_indices
                ).unsqueeze(1)
                causal_mask = (rel_pos < 0).float() * float("-inf")
                window_attn_weights = window_attn_weights + causal_mask.unsqueeze(
                    0
                ).unsqueeze(0)
            ...
            if attention_mask is not None:
                ...
                window_mask = window_mask.unsqueeze(1).unsqueeze(
                    2
                )  # [batch_size, 1, 1, window_size]
                window_attn_weights = window_attn_weights.masked_fill(
            ...
            window_context = torch.matmul(
                window_attn_probs, window_v
            )  # [batch_size, num_heads, window_q_len, head_dim]

            # Transpose and reshape back
            window_output = window_context.transpose(1, 2).reshape(
                batch_size, len(window_q_indices), -1
            )
        ...
        overlap_counts = overlap_counts.unsqueeze(0).unsqueeze(
            2
        )  # [1, local_seq_len, 1]
        local_output = local_output / overlap_counts

        # Combine global and local outputs
        global_output = global_context.transpose(1, 2).reshape(
            batch_size, self.global_tokens, -1
        )
