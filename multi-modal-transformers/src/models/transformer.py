...
from .encoders import AudioEncoder, ImageEncoder, TextEncoder
from .fusion import AttentionalPooling, CrossModalFusion
...
class MultiModalTransformerConfig:
    ...
    # Fusion parameters
    fusion_type: str = (
        "cross_attention"  # Options: cross_attention, concat, sum, weighted
    )
    fusion_layers: int = 2
...
class MultiModalTransformerLayer(nn.Module):
    ...
    def __init__(self, config: MultiModalTransformerConfig):
        ...
        # Layer normalization
        self.text_layer_norm1 = LayerNorm(
            config.text_hidden_size, eps=config.layer_norm_eps
        )
        self.text_layer_norm2 = LayerNorm(
            config.text_hidden_size, eps=config.layer_norm_eps
        )
        self.text_layer_norm3 = LayerNorm(
            config.text_hidden_size, eps=config.layer_norm_eps
        )

        self.image_layer_norm1 = LayerNorm(
            config.image_hidden_size, eps=config.layer_norm_eps
        )
        self.image_layer_norm2 = LayerNorm(
            config.image_hidden_size, eps=config.layer_norm_eps
        )
        self.image_layer_norm3 = LayerNorm(
            config.image_hidden_size, eps=config.layer_norm_eps
        )

        self.audio_layer_norm1 = LayerNorm(
            config.audio_hidden_size, eps=config.layer_norm_eps
        )
        self.audio_layer_norm2 = LayerNorm(
            config.audio_hidden_size, eps=config.layer_norm_eps
        )
        self.audio_layer_norm3 = LayerNorm(
            config.audio_hidden_size, eps=config.layer_norm_eps
        )
    ...
    def forward(
        ...
        if self.config.attention_type == "standard":
            ...
            text_features = self.text_self_attention(
                text_features, attention_mask=text_attention_mask
            )
        ...
        if self.config.attention_type == "standard":
            ...
            image_features = self.image_self_attention(
                image_features, attention_mask=image_attention_mask
            )
...
class MultiModalTransformer(nn.Module):
    ...
    def __init__(self, config: MultiModalTransformerConfig):
        ...
        self.text_position_embeddings = PositionalEncoding(
            config.text_hidden_size, max_len=config.max_text_length
        )

        self.image_position_embeddings = PositionalEncoding(
            config.image_hidden_size,
            max_len=(config.image_size // config.image_patch_size) ** 2
            + 1,  # +1 for CLS token
        )

        self.audio_position_embeddings = PositionalEncoding(
            config.audio_hidden_size, max_len=config.audio_sequence_length
        )

        # Modality tokens (learnable type embeddings)
        if config.use_modality_tokens:
            self.modality_tokens = nn.Parameter(torch.randn(3, config.hidden_size))
    ...
    def forward(
        ...
        if text is not None:
            ...
            if self.config.use_modality_tokens:
                modality_token = (
                    self.modality_tokens[0].unsqueeze(0).expand(batch_size, 1, -1)
                )
                ...
                if text_attention_mask is not None:
                    ...
                    text_attention_mask = torch.cat(
                        [modality_token_mask, text_attention_mask], dim=1
                    )
        else:
        ...
        if image is not None:
            ...
            if self.config.use_modality_tokens:
                modality_token = (
                    self.modality_tokens[1].unsqueeze(0).expand(batch_size, 1, -1)
                )
                ...
                if image_attention_mask is not None:
                    ...
                    image_attention_mask = torch.cat(
                        [modality_token_mask, image_attention_mask], dim=1
                    )
        else:
            # Create empty tensor with proper dimensions for absent modality
        ...
        if audio is not None:
            ...
            if self.config.use_modality_tokens:
                modality_token = (
                    self.modality_tokens[2].unsqueeze(0).expand(batch_size, 1, -1)
                )
                ...
                if audio_attention_mask is not None:
                    modality_token_mask = torch.ones(batch_size, 1, device=device)
                    audio_attention_mask = torch.cat(
                        [modality_token_mask, audio_attention_mask], dim=1
                    )
        else:
            # Create empty tensor with proper dimensions for absent modality
