    ...
    def __init__(self, config):
        ...
        if self.fusion_type == "cross_attention":
            self.fusion_layers = nn.ModuleList(
                [CrossAttentionFusionLayer(config) for _ in range(config.fusion_layers)]
            )
        ...
        # Components for weighted sum fusion
            ...
            self.text_projection = (
                nn.Linear(config.text_hidden_size, config.hidden_size)
                if config.text_hidden_size != config.hidden_size
                else nn.Identity()
            )

            self.image_projection = (
                nn.Linear(config.image_hidden_size, config.hidden_size)
                if config.image_hidden_size != config.hidden_size
                else nn.Identity()
            )

            self.audio_projection = (
                nn.Linear(config.audio_hidden_size, config.hidden_size)
                if config.audio_hidden_size != config.hidden_size
                else nn.Identity()
            )
        ...
        # Components for gated fusion
            ...
            self.text_gate = nn.Sequential(
                nn.Linear(config.hidden_size * 3, config.hidden_size), nn.Sigmoid()
            )
    ...
    def _cross_attention_fusion(
        ...
        if self.config.use_modality_tokens:
            ...
            fusion_features = torch.cat(
                [text_features[:, 0:1], image_features[:, 0:1], audio_features[:, 0:1]],
                dim=1,
            )
            ...
            fusion_features = torch.cat(
                [text_features, image_features, audio_features], dim=1
            )
            ...
            if (
                ...
                fusion_attention_mask = torch.ones(
                    batch_size,
                    text_seq_len + image_seq_len + audio_seq_len,
                    device=device,
                )
    ...
    def _concat_fusion(
        ...
        if self.config.use_modality_tokens:
            ...
            concat_features = torch.cat(
                [text_pooled, image_pooled, audio_pooled], dim=-1
            )
