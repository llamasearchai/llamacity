...
class MultiQueryAttentionalPooling(nn.Module):
    ...
    def __init__(
        ...
        self.queries = nn.Parameter(torch.zeros(1, num_queries, num_heads, self.head_dim))
    ...
    def forward(
        ...
        for query_idx in range(self.num_queries):
            ...
            attention_scores = torch.matmul(current_q, k.unsqueeze(1).transpose(3, 4)) * scale
            ...
            current_context = current_context.transpose(1, 2).reshape(batch_size, 1, self.hidden_size)
    ...
    def forward(
        ...
        if window_masks is not None:
            ...
            window_combined_mask = torch.stack(
                [mask.sum(dim=1) > 0 for mask in window_masks],
                dim=1
            ).float()
        else:
