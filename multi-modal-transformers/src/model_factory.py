...
from src.config import DecoderConfig, EncoderConfig, ModelConfig, MultiModalConfig
...
class ModelFactory:
    ...
    @staticmethod
    def create_multi_modal_transformer(
    ...
    def from_config(cls, config: ModelConfig) -> nn.Module:
        ...
        if config.model_type == "encoder_only":
            if config.multi_modal_config is not None:
                ...
                assert (
                    config.encoder_config is not None
                ), "Encoder config must be provided"
...
def create_model(
    model_name_or_path: str, model_type: Optional[str] = None, **kwargs
) -> nn.Module:
    """
    ...
    if os.path.isdir(model_name_or_path) and os.path.exists(
