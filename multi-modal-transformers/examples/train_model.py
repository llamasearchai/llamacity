...
def train_step(
    ...
    if args.task == "retrieval" and "scores" in outputs:
        ...
        metrics.update(compute_captioning_metrics(outputs["logits"], batch["caption_ids"]))
...
def compute_vqa_metrics(logits: torch.Tensor, targets: torch.Tensor) -> Dict[str, float]:
    """
...
def compute_captioning_metrics(logits: torch.Tensor, targets: torch.Tensor) -> Dict[str, float]:
    """
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Training")):
            ...
            if (global_step + 1) % args.log_interval == 0:
                log_metrics(
                    metrics={k: np.mean(v[-args.log_interval:]) for k, v in train_metrics.items()},
                    step=global_step,
                    prefix="train"
                )
            ...
            if (global_step + 1) % args.eval_interval == 0:
                ...
                if args.task == "captioning":
                    ...
                    current_metric = val_metrics.get("r@1", val_metrics.get("accuracy", 0.0))
                    is_best = current_metric > best_val_metric
