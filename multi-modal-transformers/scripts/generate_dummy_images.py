...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate dummy images for testing")
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--num_images", type=int, default=8, help="Number of images to generate"
    )
    parser.add_argument(
        "--image_size", type=int, default=224, help="Size of images (height and width)"
    )
...
def generate_random_image(size, index):
    ...
    draw.rectangle(
        [text_bg_x1, text_bg_y1, text_bg_x2, text_bg_y2], fill=(255, 255, 255)
    )
